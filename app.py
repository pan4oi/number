...
ing gray image.convert("L")
ing resized ing gray.resize((8, 8))

#Convert to numpy array and invert if needed
ing array np.array(ing resized)

#If background is dark, invert
11 np.mean(ting array) > 128:
Ing array 255 1ng aгrey

#Normalize like the training data
ing arraying array / 16.0
ing flating array.flatten().reshape(1, 1)
if model is not None:
# Make prediction
prediction model.predict(ing_flat)[0]
st.write("## Prediction: **(prediction)***")
#Show probabilities
probs sadel.predict proba(ing flat)[0]
st.write("### Probabilities:")
for i, prob in enumerate (probs):
st.write("Digit (1): (prob:.25)")
else:
#Fallbuck: simple threshold-based recognition
